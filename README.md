...
[![Tests](https://img.shields.io/badge/tests-149%20passed-3fb950)](#-тестирование)
...
- **Прокрутка протяжкой**: доску, Гант, ряды вкладок и широкие таблицы можно листать, зажав их мышью, — как панорамирование в графе веток; после броска лента доезжает по инерции. Когда карточку тащат к краю доски, она подкручивается сама — колонку за пределами экрана видно, куда бросать.
...
### Импорт проектов (Excel · MS Project · Trello)
На дашборде — кнопка **«Импорт из файла»**. Поддерживаются:

- **Excel** (`.xlsx`) — каждая строка становится задачей. Распознаются столбцы (рус./англ., в любом порядке): `Название`, `Описание`, `Статус`, `Тип`, `Приоритет`, `Исполнитель` (e-mail, несколько — через запятую), `Оценка (ч)`, `Начало`, `Срок`, `Колонка`, `Родитель`. Если задан столбец `Колонка` — создаются одноимённые колонки доски (как в Trello); иначе колонки формируются по статусу. `Родитель` (название другой задачи) восстанавливает дерево подзадач. Шаблон: **«Скачать шаблон Excel»** в диалоге или `GET /api/import/template`.
- **MS Project** (`.xml`, экспорт «Сохранить как → XML») — задачи, статус по `% завершения`, оценка из трудозатрат, приоритет, а иерархия восстанавливается по уровню структуры (Outline Level).
- **Trello** — отдельная кнопка **«Импорт из Trello»** (списки → колонки, карточки → задачи, чек-листы переносятся). Ключ/токен можно сохранить (хранятся в зашифрованном виде).

Исполнители из файла сопоставляются с существующими пользователями по e-mail и добавляются в участники проекта.
