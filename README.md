...
[![Tests](https://img.shields.io/badge/tests-208%20passed-3fb950)](#-тестирование)
...
- **Доска помнит, где вы остановились**: прокрутка доски и её колонок восстанавливается после перехода в задачу и обратно (в пределах вкладки).

### Для менеджера
- **Канбан** с кастомными колонками и **WIP-лимитами** (превышение подсвечивается).
- **Спринты и burndown**: итерации с датами и целью, график сгорания (часы или задачи) и прогресс.
- **Правила доски**: «карточка попала в колонку → сменить статус, назначить исполнителя, повесить метку».
- **Живая доска**: изменения коллег появляются сами, без перезагрузки. Браузер раз в несколько секунд спрашивает короткий отпечаток доски и перерисовывает её, только когда он изменился; скрытая вкладка не опрашивает вовсе.
- **Приём заявок по ссылке**: сторонний человек заполняет форму — в проекте появляется задача.
...
- **Компании**: сотрудники и проекты разделены по контурам — из чужой компании проект не виден и в состав его участников не попасть. Компании заводят и наполняют администраторы и менеджеры (раздел «Компании» в админке).
- **Проект как подзадача**: ветка, доросшая до отдельного проекта, привязывается к задаче и показывает свой прогресс прямо в её карточке.

### Для аналитика
- **Нагрузка команды**: открытые задачи, часы и просрочки по каждому; часы общих задач делятся поровну.
...
- **Сохранённые фильтры** — общие для списка и доски, применяются в один клик.
- **Массовые операции**: выбрать задачи и разом сменить статус, приоритет, тип, исполнителя или метку.
...
- 💬 **Комментарии** ветками (ответы в треде) с **@упоминаниями** участников.
...
prisma/schema.prisma            — модели: User, Project, Task (self-relation = ветки),
                                  Company (контур видимости), TaskProjectLink (проект
                                  как подзадача),
...
src/lib/access.ts               — проверки доступа: членство в проекте, роли, компания
src/lib/subprojects.ts          — проекты, привязанные к задачам как подзадачи
src/lib/actions/                — server actions (auth, projects, tasks, board, admin,
                                  companies, subprojects, templates, filters, tokens,
                                  webhooks)
...
- [x] Вложенные ответы (threads) в комментариях
- [x] Зависимости задач стрелками прямо на диаграмме Ганта
- [ ] Фильтры Ганта по исполнителю и статусу
- [x] Живые обновления доски (опрос отпечатка)
