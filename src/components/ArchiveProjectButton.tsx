...
export function ArchiveProjectButton({
  ...
  function run() {
    startTransition(async () => {
      await toggleProjectArchiveAction(projectId);
      setAsking(false);
    });
  }
  ...
  return (
    <>
      <button
        ...
        aria-label={archived ? "Восстановить из архива" : "В архив"}
        disabled={pending}
        onClick={(e) => {
          e.preventDefault();
          // Восстановление безобидно — подтверждаем только уход в архив
          if (archived) run();
          else setAsking(true);
        }}
      ...
      <ConfirmDialog
        ...
        message="Проект скроется из списка активных. Данные сохранятся, вернуть его из архива можно в любой момент."
